...
def scrape_profile(player_profile, hyperlink, queue=None):
    ...
    try:
        matches = get_matches(driver,player_profile)
        driver.get(url)
        try:
            ...
            return (hyperlink, meta_content, total_games, matches)
...
def scrap_row(content,row):
    tds = row.find_elements(By.TAG_NAME,'td')
    content['Date'].append(get_element(tds,0))
    content['Map'].append(get_element(tds,2))
    content['Score'].append(get_element(tds,3))
    content['Kills'].append(int(get_element(tds,5)))
    content['Deaths'].append(int(get_element(tds,6)))
    content['Assists'].append(int(get_element(tds,7)))
    content['HS'].append(int(get_element(tds,9)))
    content['ADR'].append(int(get_element(tds,10)))
    content['Rating'].append(float(get_element(tds,19)))

def get_matches(driver : webdriver.Chrome,player_profile):
    content = {'Date':[],'Map':[],'Score':[],'Kills':[],'Deaths':[],'Assists':[],'HS':[],'ADR':[],'Rating':[]}
    driver.get(player_profile+'#/matches')
    try:
        rows = driver.find_elements(By.CLASS_NAME, "p-row.js-link")
        for row in rows:
            scrap_row(content,row)
    except Exception as e:
        print(f"Error: {str(e)}")
    return content


def get_stats(user_profile_hyper_link, meta_content, total_games, find_win_percentage_regex,
